...
StatusOr<Layout> PjRtCApiClient::GetDefaultLayout(
  ...
  args.dims = dims.data();
  args.num_dims = dims.size();
  RETURN_STATUS_IF_PJRT_ERROR(extension->pjrt_client_get_default_layout(&args),
                              c_api);
  ...
  RETURN_STATUS_IF_PJRT_ERROR(
      extension->memory_layout_serialize(&serialize_args), c_api);
  ...
  // Clean up `PJRT_Layouts_MemoryLayout` and `PJRT_SerializedLayout`.
