...
typedef struct PJRT_SerializedLayout PJRT_SerializedLayout;
...
struct PJRT_Layouts_MemoryLayout_Serialize_Args {
  ...
  PJRT_SerializedLayout* serialized_layout;  // backs serialized_bytes.

  // cleanup fn must be called to free the backing memory for serialized_bytes.
  // Should only be called once on serialized_layout.
  void (*serialized_layout_deleter)(PJRT_SerializedLayout* s_layout);  // out
};
...
typedef struct PJRT_Layouts_Extension {
  ...
  PJRT_Layouts_MemoryLayout_Serialize* memory_layout_serialize;

  PJRT_Layouts_PJRT_Client_GetDefaultLayout* pjrt_client_get_default_layout;
