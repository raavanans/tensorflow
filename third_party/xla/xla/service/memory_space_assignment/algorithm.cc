...
absl::StatusOr<MsaAlgorithm::Result> MsaAlgorithm::AllocateAllocationValues(
  ...
  for (AllocationValue& allocation_value : allocation_values) {
    ...
    for (AllocationValue::Use& use : allocation_value.uses()) {
      ...
      result_mark(AllocateAllocationValueUse(
                      allocation_value, use, preferred_offset, previous_use,
                      all_use_times, definition_time,
                      require_no_copy_alternate_mem_allocation, request),
                  result);
      ...
      UpdatePreferredOfsetForComputation(
          allocation_value, use, /*use_time=*/request.end_time,
          allocation_values, preferred_offset_for_computation);
    }
  }
  return result;
}

MsaAlgorithm::Result MsaAlgorithm::AllocateAllocationValueUse(
  ...
  request = CreateAllocationRequest(
      allocation_value, use, previous_use, preferred_offset, definition_time,
...
void MsaAlgorithm::UpdatePreferredOfsetForComputation(
  ...
  for (const HloPosition& aliased_position : use.aliases) {
    // bool matching_computations = true;
    // for (auto& computation :
    //      aliased_position.instruction->called_computations()) {
    //   if (computation != allocation_value.computation()) {
    //     matching_computations = false;
    //     break;
    //   }
    // }
    // if (!matching_computations) {
    //   VLOG(1) << "Mismatch between the computations for allocation value "
    //           << allocation_value.ToString() << " and its use "
