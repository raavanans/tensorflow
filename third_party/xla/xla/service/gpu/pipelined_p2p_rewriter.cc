...
HloInstruction* MaySkipTrivialTuple(const HloInstruction* op) {
  return MaySkipTrivialTuple(const_cast<HloInstruction*>(op));
}

// Finds a consecutive block of balanced SendDone/RecvDone in the while-loop,
// assuming while_init is a tuple.
...
FindConsecutiveAndBalanceBlockOfSendDoneRecvDone(
  ...
  if (opnd_tot > 0) {
    ...
    for (int64_t i = opnd_start + opnd_tot; i < while_init->operand_count();
         ++i) {
      const HloInstruction* op = while_init->operand(i);
      if (op->opcode() == HloOpcode::kRecvDone ||
          op->opcode() == HloOpcode::kSendDone) {
  ...
  return PipelinedP2PInfo{opnd_start, opnd_start + opnd_tot};
}
...
std::optional<PipelinedP2PInfo> FindPipelinedP2P(
  ...
  int64_t opnd_start = pipelined_p2p_info->opnd_start;
  int64_t opnd_end = pipelined_p2p_info->opnd_end;
  ...
  for (int64_t i = opnd_start; i < opnd_end; ++i) {
    ...
    if (op->opcode() == HloOpcode::kRecvDone) {
      ...
      if (!(while_body->parameter_instruction(0), i)) {
...
absl::Status RemoveDoneOpsAndUpdateSequence(
  ...
  for (auto op : ops) {
    if (op->opcode() == HloOpcode::kTuple) {
      InstructionVector tobe_removed;
      HloInstruction* tuple_op = op;
      op = MaySkipTrivialTuple(tuple_op);
      tobe_removed.push_back(tuple_op);
      for (auto opnd : tuple_op->mutable_operands()) {
        tobe_removed.push_back(opnd);
      }
      for (auto opnd : tobe_removed) {
