...
class SparseCoreEmbeddingConfig:
  ...
  def __init__(
      ...
      max_ids_per_table: Dict[str, int],
      max_unique_ids_per_table: Dict[str, int],
      allow_id_dropping: bool = False,
  ):
    self.disable_table_stacking = disable_table_stacking
    self.max_ids_per_chip_per_sample = max_ids_per_chip_per_sample
    self.max_ids_per_table = max_ids_per_table
    self.max_unique_ids_per_table = max_unique_ids_per_table
    self.allow_id_dropping = allow_id_dropping

  def __repr__(self):
    return (
        "SparseCoreEmbeddingConfig(disable_table_stacking={},"
        " max_ids_per_chip_per_sample={}, max_ids_per_table={},"
        " max_unique_ids_per_table={}, allow_id_dropping={})".format(
            self.disable_table_stacking,
            self.max_ids_per_chip_per_sample,
            self.max_ids_per_table,
            self.max_unique_ids_per_table,
            self.allow_id_dropping,
        )
    )
...
class TPUEmbeddingV2(tpu_embedding_base.TPUEmbeddingBase):
  ...
  DEFAULT_MAX_IDS_PER_CHIP_PER_SAMPLE = 64
  ...
  def __init__(
    ...
    if sparse_core_embedding_config is None:
      self._sparse_core_embedding_config = SparseCoreEmbeddingConfig(
          ...
          max_ids_per_chip_per_sample=self.DEFAULT_MAX_IDS_PER_CHIP_PER_SAMPLE,
          max_ids_per_table={
              table.name: self.DEFAULT_MAX_IDS_PER_TABLE
              for table in self._table_config
          },
          max_unique_ids_per_table={
              table.name: self.DEFAULT_MAX_UNIQUE_IDS_PER_TABLE
              for table in self._table_config
          },
      )
      logging.warning(
    ...
    self._s = self._stack_tables_with_same_table_dim_and_optimizer(
        self._table_config,
        self._flat_features,
  ...
  def _update_sparse_core_buffer_size_after_table_stacking(self):
    ...
    for table_name in self._stacked_table_to_tables:
      if table_name not in self._sparse_core_embedding_config.max_ids_per_table:
        logging.warning(
            "Table %s is not found in max_ids_per_table provided by"
            " SparseCoreEmbeddingConfig. Using default value 256.",
            table_name,
        )
        self._table_to_max_ids_per_sparse_core[table_name] = 256
      else:
      ...
      if (
        ...
        self._table_to_max_unique_ids_per_sparse_core[table_name] = 256
      else:
  ...
  def _stack_tables_with_same_table_dim_and_optimizer(
    ...
    if sparse_core_embedding_config:
      disable_table_stacking = (
          sparse_core_embedding_config.disable_table_stacking
      )
  ...
  def compute_sparse_core_stats(
    ...
    s = TPUEmbeddingV2._stack_tables_with_same_table_dim_and_optimizer(
        table_config,
        flat_features,
        num_tpu_chips,
        num_sc_per_chip,
